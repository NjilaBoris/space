...
export default function AnimatedTitles({
    ...
    () => {
      ...
      titlesEls.forEach((titleEl, index) => {
        ...
        const titleContainerInitialX =
          index === 1 && titlesEls.length >= 3 ? -100 : 100;
            ...
            split.chars.forEach((char, i) => {
              ...
              if (self.progress >= startPoint) {
                charProgress = Math.min(
                  1,
                  (self.progress - startPoint) / duration
                );
              }
  ...
  return (
    <section
      className="animated-titles relative w-full overflow-hidden"
      ref={containerRef}
    >
      {titles.map((t, i) => (
        <div className="title" key={i}>
          <div className="title-container">
            <h1 className="heading-title">{t}</h1>
          </div>
        </div>
      ))}
    </section>
